...
* Для автоматического распределения по каналам неизвестен ожидаемый поток учеников; Допускаем 45 на тариф;
* Распределение должно быть равномерным или по мере наполнения? Ожидаемый поток неизвестен - допускаем 2;
* Закрытие чата через 10 дней требует дополнительную логику. Реализовать процессом или продуктом?
