    ...
    - name: Deploy script to remove SSH port 22 rule
      copy:
        ...
        content: |
          #!/bin/bash
          sleep 60
          sudo sed -i '/tcp dport { 22, 80, 443 }/s/22, //g' /etc/sysconfig/nftables.conf
          sudo nft -f /etc/sysconfig/nftables.conf
          rm -f /tmp/remove_ssh_port_22.sh
      # Отправляет скрипт, который удаляет правило для порта 22 через 60 секунд.

    - name: Set permissions for script to remove SSH port 22 rule
      file:
        path: /tmp/remove_ssh_port_22.sh
        mode: '0755'
      # Обновляет права на файл.

    - name: Execute script to schedule removal of SSH port 22
      command: /tmp/remove_ssh_port_22.sh &
      async: 0
      poll: 0
      # Запускает скрипт для удаления правила порта 22.
